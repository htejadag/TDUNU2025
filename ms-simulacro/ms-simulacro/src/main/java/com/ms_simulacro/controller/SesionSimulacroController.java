...
@RestController
@RequestMapping(AppConstants.API_PATH + "/sesiones-simulacro")
@RequiredArgsConstructor
public class SesionSimulacroController {

    private final SesionSimulacroService service;

    @GetMapping
    public ResponseEntity<ApiResponse<List<SesionSimulacroDto>>> getAll() {
        List<SesionSimulacroDto> list = service.getAll();
        return ResponseEntity.ok(ApiResponse.success(list, AppConstants.SUCCESS_MESSAGE));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<SesionSimulacroDto>> getById(@PathVariable Long id) {
        SesionSimulacroDto dto = service.getById(id);
        if (dto != null) {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<SesionSimulacroDto>> create(@RequestBody SesionSimulacroDto dto) {
        SesionSimulacroDto saved = service.create(dto);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success(saved, AppConstants.CREATED_MESSAGE));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<SesionSimulacroDto>> update(@PathVariable Long id,
            @RequestBody SesionSimulacroDto dto) {
        SesionSimulacroDto updated = service.update(id, dto);
        if (updated != null) {
