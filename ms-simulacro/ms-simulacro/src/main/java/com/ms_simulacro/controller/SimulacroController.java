...
@RestController
@RequestMapping(AppConstants.API_PATH + "/simulacros")
...
public class SimulacroController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<SimulacroDto>>> getAll() {
        List<SimulacroDto> list = service.getAll();
        return ResponseEntity.ok(ApiResponse.success(list, AppConstants.SUCCESS_MESSAGE));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<SimulacroDto>> getById(@PathVariable Long id) {
        SimulacroDto dto = service.getById(id);
        if (dto != null) {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<SimulacroDto>> create(@RequestBody SimulacroDto dto) {
        SimulacroDto saved = service.create(dto);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success(saved, AppConstants.CREATED_MESSAGE));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<SimulacroDto>> update(@PathVariable Long id, @RequestBody SimulacroDto dto) {
        SimulacroDto updated = service.update(id, dto);
        if (updated != null) {
